...
forward_backward_sweep_shooting <- function(parameter_df, emissions_df, economic_df, scenario, 
  ...
  for (iter in 1:max_iterations) {
    ...
    # === UPDATE CONTROLS (SIMPLE VERSION) ===
    ...
    for (i in 1:n_years) {
      ...
      # === MITIGATION CONTROL (ADAPTIVE BUFFER PROJECTION) ===
      ...
      # Adaptive buffer projection
      if (um_unconstrained <= 0) {
        new_mitig[i] <- 0
      } else if (um_unconstrained >= baseline_emissions[i]) {
        
        # Calculate how much the solution "wants" to violate the constraint
        excess_demand <- um_unconstrained - baseline_emissions[i]
        
        # Adaptive buffer based on the excess demand and shadow price
        # Larger excess demand = larger buffer needed for numerical stability
        base_buffer <- 1e-6
        adaptive_component <- min(excess_demand * 1e-3, 0.01)  # Cap at 1%
        
        # Scale buffer by constraint magnitude to maintain relative precision
        relative_buffer <- (base_buffer + adaptive_component) * baseline_emissions[i]
        
        new_mitig[i] <- baseline_emissions[i] - relative_buffer
