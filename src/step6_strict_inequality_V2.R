...
forward_backward_sweep_shooting <- function(parameter_df, emissions_df, economic_df, scenario, 
  ...
  # Calculate comprehensive cost breakdown - YEARLY VECTORS
  final_temperature <- temperature_anomaly[n_years]
  discount_factors <- exp(-disc_rate * years_rel)
  
  # Annual costs (these are now the yearly vectors we want to return)
  ...
  return(list(
    ...
    # Total costs (backward compatibility)
...
test_step6_shooting <- function(test_epsilon_sensitivity = FALSE) {
  ...
  if (!is.null(result)) {
    ...
    # NEW: Display some yearly cost information
    cat("\n=== YEARLY COST VECTORS (Sample) ===\n")
    cat("First 5 years mitigation costs:", sprintf("%.2f", result$mitig_costs_annual[1:5]), "\n")
    cat("First 5 years CDR costs:", sprintf("%.2f", result$remov_costs_annual[1:5]), "\n")
    cat("First 5 years temperature costs:", sprintf("%.2f", result$temp_costs_annual[1:5]), "\n")
    cat("First 5 years total costs:", sprintf("%.2f", result$total_costs_annual[1:5]), "\n")
    cat("Length of cost vectors:", length(result$total_costs_annual), "years\n")
  }
