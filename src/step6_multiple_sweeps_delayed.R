# Step 6: Parameter Sensitivity Analysis with Deployment Delays
# Core function with added mitigation and CDR delay parameters
...
#' Forward-Backward Sweep Function with Deployment Delays
forward_backward_sweep_shooting_delayed <- function(parameter_df_row, emissions_df, economic_df, scenario, 
                                                    terminal_adjoint, 
...
#' Shooting Method with Deployment Delays
real_data_shooting_method_delayed <- function(parameter_df_row, emissions_df, economic_df, scenario,
                                              mitigation_delay_years = 0, 
                                              cdr_delay_years = 0) {
  ...
  # Test bounds
  result_low <- forward_backward_sweep_shooting_delayed(parameter_df_row, emissions_df, economic_df, 
                                                        scenario, lambda_low,
  ...
  result_high <- forward_backward_sweep_shooting_delayed(parameter_df_row, emissions_df, economic_df, 
  ...
  for (iteration in 1:max_shooting_iterations) {
    ...
    result_new <- forward_backward_sweep_shooting_delayed(parameter_df_row, emissions_df, economic_df, 
                                                          scenario, lambda_new,
                                                          mitigation_delay_years, cdr_delay_years)
...
#' Main Parameter Sensitivity Analysis Function with Deployment Delays
run_parameter_sensitivity_analysis_delayed <- function(parameter_df, 
                                                       emissions_df,
                                                       economic_df,
  ...
  cat("=== Parameter Sensitivity Analysis with Deployment Delays ===\n")
  cat("Number of parameter samples:", n_samples, "\n")
  cat("Scenario:", scenario, "\n")
  cat("Mitigation deployment delay:", mitigation_delay_years, "years\n")
  cat("CDR deployment delay:", cdr_delay_years, "years\n")
  ...
  for (i in 1:n_samples) {
    ...
    tryCatch({
      result <- real_data_shooting_method_delayed(current_params, emissions_df, economic_df, 
                                                  scenario, mitigation_delay_years, cdr_delay_years)
...
# =============================================================================
# VISUALIZATION FUNCTIONS - Same as original multiple_sweeps script
# =============================================================================
...
# 
# # Run sensitivity analysis with no delays (same as original)
# results_no_delay <- run_parameter_sensitivity_analysis_delayed(parameter_df, emissions_df, economic_df)
# 
# # Run sensitivity analysis with 10 year mitigation delay
# results_mitig_delay <- run_parameter_sensitivity_analysis_delayed(parameter_df, emissions_df, economic_df,
#                                                                   mitigation_delay_years = 10)
# 
# # Run sensitivity analysis with both delays
# results_both_delays <- run_parameter_sensitivity_analysis_delayed(parameter_df, emissions_df, economic_df,
#                                                                   mitigation_delay_years = 10,
#                                                                   cdr_delay_years = 5)
# 
# # Create visualisations (same plots as original)
# create_combined_dashboard(results_no_delay$successful_runs)
# create_combined_dashboard(results_mitig_delay$successful_runs)
# create_combined_dashboard(results_both_delays$successful_runs)

cat("=== Parameter Sensitivity Analysis with Deployment Delays Ready ===\n")
cat("Main function: run_parameter_sensitivity_analysis_delayed(parameter_df, emissions_df, economic_df, \n")
cat("                                                         mitigation_delay_years = 0, cdr_delay_years = 0)\n")
cat("Default delays are 0 years (immediate deployment)\n")
cat("Same plotting functions as original multiple_sweeps script\n")


# Run sensitivity analysis with 20 year delays for both mitigation and CDR
multi_results <- run_parameter_sensitivity_analysis_delayed(parameter_df, emissions_df, economic_df,
